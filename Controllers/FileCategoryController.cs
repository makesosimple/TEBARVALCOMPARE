    ...
    public class FileCategoryController : Controller
    {
        ...
        public async Task<IActionResult> Create([Bind("FileCategoryID,FileCategoryTitle,FileCategoryFolderName,FileCategoryDescription,ParentFileCategoryID,UserID,CreationDate,UpdateDate,DeletionDate")] FileCategory fileCategory)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    fileCategory.CreationDate = DateTime.Now;
                    fileCategory.UserID = _userManager.GetUserId(HttpContext.User);

                    _context.Add(fileCategory);
                    await _context.SaveChangesAsync();
                    TempData["SuccessTitle"] = "BAŞARILI";
                    TempData["SuccessMessage"] = $" {fileCategory.FileCategoryID} numaralı kayıt başarıyla oluşturuldu.";
                    return RedirectToAction(nameof(Index), new { id = fileCategory.FileCategoryID.ToString() });
                }
                catch (Exception ex)
                {
