    ...
    public class ContractorTypeController : Controller
    {
        ...
        public async Task<IActionResult> Create([Bind("ContractorTypeID,ContractorTypeTitle,ContractorTypeDescription,UserID,CreationDate,UpdateDate,DeletionDate")] ContractorType contractorType)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    contractorType.CreationDate = DateTime.Now;
                    contractorType.UserID = _userManager.GetUserId(HttpContext.User);

                    _context.Add(contractorType);
                    await _context.SaveChangesAsync();
                    TempData["SuccessTitle"] = "BAŞARILI";
                    TempData["SuccessMessage"] = $" {contractorType.ContractorTypeID} numaralı kayıt başarıyla oluşturuldu.";
                    return RedirectToAction(nameof(Index), new { id = contractorType.ContractorTypeID.ToString() });
                }
                catch (Exception ex)
                {
        ...
        public async Task<IActionResult> Edit(int id, [Bind("ContractorTypeID,ContractorTypeTitle,ContractorTypeDescription,UserID,CreationDate,UpdateDate,DeletionDate")] ContractorType contractorType)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    TempData["SuccessTitle"] = "BAŞARILI";
                    TempData["SuccessMessage"] = $"{contractorType.ContractorTypeID} numaralı kayıt başarıyla düzenlendi.";
                }
        ...
        public async Task<IActionResult> DeleteConfirmed(int id)
        {
            ...
            try
            {
                ...
                await _context.SaveChangesAsync();
                TempData["SuccessTitle"] = "BAŞARILI";
                TempData["SuccessMessage"] = $"{contractorType.ContractorTypeID} numaralı kayıt başarıyla silindi.";
                return RedirectToAction(nameof(Index));
            }
